...
class World():
    ...
    def update_state(self, world_state, action, agent_host):
        ...
        if world_state.number_of_observations_since_last_state > 0:
            if action == 4:
                if self.holding_wheat:  # Punish redundantly choosing show wheat
                    reward = -200
                else:
                    reward += 200
                    self.holding_wheat = True
            elif action == 5 and self.holding_wheat:
                reward -= 50      # If holding wheat, putting down the wheat will be punished
